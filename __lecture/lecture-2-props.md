...
- The same component can work with **different data** (think functions)
...
<RecommendedArtist
  ...
  artistName="Skrillex"
  numOfSongs={9}
  //NUMBERS MUST BE IN {} or it assumes it's a different type(string?)
/>
<RecommendedArtist
...
// using deconstructed:
function VideoPlayer({videoUrl, width, height, descript}) {
  return (
    <div>
      <video
        src= {videoUrl}
...
//function Page () {
  //return(
//<div>
//  <VideoPlayer videoUrl='bleh' width={123} height={456} descript='beans'>
//</div>)
//}

//OR not-deconstructed
function VideoPlayer(props) {
  return (
    <div>
      <video
...
//tweet buttons would be their own compontent etc
```
...
//also could just pass pet to PetInfo if it deconstructs on that end 
//<PetInfo pet={pet}>
```
